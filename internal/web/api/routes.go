...
func RegisterRoutes(e *echo.Echo, signalService *signal.Service, alarmService *alarm.AlarmService, asyncClient *asynq.Client) {
	...
	if authConfig.OAuth.Enabled {
		...
		if err != nil {
			...
			e.GET("/auth/login", oauthHandlers.LoginHandler)
			e.GET("/auth/callback", oauthHandlers.CallbackHandler)
			e.POST("/auth/logout", oauthHandlers.LogoutHandler)
			e.GET("/auth/status", oauthHandlers.StatusHandler)
		}
	}
