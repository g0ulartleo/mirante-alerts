...
func (a *AuthCommand) Run(args []string) error {
	...
	// Check if the server supports OAuth
	if !checkOAuthSupport(apiHost) {
		fmt.Println("Server does not support OAuth. Please use the 'config' command with an API key.")
		return nil
	}
...
func checkOAuthSupport(apiHost string) bool {
	...
	// If we get any response (even 401), OAuth is supported
