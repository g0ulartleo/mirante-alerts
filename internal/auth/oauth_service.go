...
func NewOAuthService(authConfig *config.AuthConfig) (*OAuthService, error) {
	...
	// Read OAuth secrets from environment variables
...
func (s *OAuthService) GetUserInfo(ctx context.Context, token *oauth2.Token) (*UserInfo, error) {
	...
	// For GitHub, we need to get the primary email separately
	if s.config.OAuth.Provider == "github" && userInfo.Email == "" {
		if err := s.getGitHubEmail(ctx, client, &userInfo); err != nil {
...
func (s *OAuthService) getGitHubEmail(ctx context.Context, client *http.Client, userInfo *UserInfo) error {
...
func (s *OAuthService) ValidateJWT(tokenString string) (*Claims, error) {
	...
	if claims, ok := token.Claims.(*Claims); ok && token.Valid {
		// Check if the email is still allowed (in case config changed)
