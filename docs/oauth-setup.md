...
OAuth authentication provides several benefits over API key authentication:

- **Centralized Access Control**: Use your existing identity provider (Google/GitHub)
- **Fine-grained Permissions**: Control access by email domain or specific emails
- **Better Security**: No need to manage and distribute API keys
- **Audit Trail**: Better tracking of who accessed the system
- **Easy Revocation**: Disable access by removing users from allowed lists
...
oauth:
  ...
  session_timeout: "24h"
api_key: ""  # Leave empty when using OAuth
```
